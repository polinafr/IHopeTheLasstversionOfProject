...
namespace BL
{
    class Apriory
    {
        //string _FilePath;
        List<List<Good>> list;
        List<Good> DistinctValues;
        List<List<Good>> ItemSets;
        public Apriory()
        {
            //_FilePath = FilePath;
            IBucketRepository bucketRepository = new BucketRepository();
            List<Bucket> allBuckets = bucketRepository.GetAll();
            List<List<Good>> transactions = new List<List<Good>>();
            for (int i=0; i<allBuckets.Count(); i++)
            {
                transactions.Add(allBuckets[i].Goods);
            }
            ItemSets = new List<List<Good>>();
            SetDistinctValues(list);
        }

        public ItemSet GetItemSet(int length, int support, bool Candidates = false, bool IsFirstItemList = false)
        {
            List<IEnumerable<Good>> result = GetPermutations(DistinctValues, length).ToList(); //possible error
            List<List<Good>> data = new List<List<Good>>();
            foreach (var item in result)
            {
                data.Add(item.ToList());
            }
            List<Good> itemSet = new List<Good>();
        ...
        public void SetDistinctValues(List<List<Good>> values)
        {
            List<Good> data = new List<Good>();
            foreach (var item in values)
            {
                //var row = item.//item.Split(' ');
                foreach (var item2 in item)
                {
            ...
            DistinctValues = new List<Good>();
            DistinctValues.AddRange(data.OrderBy(a => a).ToList());// sort - does it really work???
        }
